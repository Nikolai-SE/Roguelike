...
Представляет реализацию абстрактного класс ячеек, является элементов игрового мира. Объекты этого класса способны отображаться на экране пользователя, а также взаимодействовать с персонажем.
...
> `enemyStrategy` - стратегия, которая определяет поведения вражеской единицы
