...
> `render(Canvas, Vector)` - отрисовывает игровой мир, главного героя и остальных персонажей.
...
> `render(Canvas, Vector)`, выполняет отрисовку объекта класса, который реализует этот интерфейс, на экране пользователя.
...
> `render(Canvas, Vector)`, выполняет отрисовку.
...
> `render(Canvas, Vector)`, отображает `HUD` на экране пользователя.
...
> `render(Canvas, Vector): void`, отрисовывает ячейку.


### SolidCell

Представляет реализацию абстрактного класса для ячеек, является элементов игрового мира. Объекты этого класса способны оторбражаться на экране пользователя, а также взаимодействовать с персонажем.
...
> `render(Canvas, Vector)`, выполняет отрисовку ячейки.
...
> `render(Canvas)`, отрисовывает объект.
...
Класс представляет собой вражескую единицу, является наследником `Unit`. Объекты этого класса способны отображаться в игровом мире и наносить урон по персонажу.

Поля:

> `behaviour: EnemyBehaviour`, стратегия, которая определяет поведения врежеской единицы.
...
Декоратор, который определяет поведение вражеской единице во время конфузии.
...
Объект этого класс является персонажем, которым управляет игрок.
