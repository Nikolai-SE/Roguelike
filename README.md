...
Абстрактный класс содержит минимальный набор методов, которые должны реализовывать класс для того, чтобы его объекты могли быть отрисованы.
...
> `normalBehaviourState: NormalBehaviourState`, ссылка на обычное состояние (состояния знают друг о друге, поскольку сами совершают переходы). Поле используется для инъекции зависимостей, чтобы создавать меньше объектов в рантайме при переключениях.
