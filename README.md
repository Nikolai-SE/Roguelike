...
### CowardBehaviourState

Класс определяет трусливое состояние врага.

Методы:

> `move(Enemy): CowardBehaviourState`, выполняет перемещение случайным образом, за исключением случаев, когда видит игрока, в этом случае выполняет перемещение от игрока.

### PassiveBehaviourState

Класс определяет пассивное состояние врага.
Такой моб не двигается, но если у него окажется менее трети здоровья, то начнёт убегать.

Методы:

> `move(Enemy): PassiveBehaviourState`, не выполняет перемещение.

### AggressiveBehaviourState

Класс определяет агрессивное состояние врага.
Такой моб двигается в сторону игрока, но если у него окажется менее трети здоровья, то начнёт убегать.

Методы:

> `move(Enemy): AggressiveBehaviourState`, выполняет перемещение случайным образом, за исключением случаев, когда видит игрока, в этом случае выполняет перемещение к игроку.

### ConfusionState

Декоратор, который определяет состояние вражеской единицы во время конфузии.
...
Декоратор состояния, заменяет стратегию перемещения на трусливую,
пока у моба не станет больше половины здоровья.
...
> `move(Enemy): EnemyBehaviourState`, выполняет перемещение согласно определенным правилам.
