...
> `build(): World`, строит мир по набору определенных параметров.

> `reset()`, заполняет параметры для постройки игрового мира значениями по умолчанию.
...
> `source(String)`, парсит характеристики для игрового мира из файла, путь к которому передается в качестве аргумента.
